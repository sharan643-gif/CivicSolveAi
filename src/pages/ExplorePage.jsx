...
export default function ExplorePage({ challenges = [], onNavigate }) {
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', background: 'rgba(255,255,255,0.03)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
          ...
          <button 
            ...
            style={{
              ...
              fontSize: '0.8rem',
              borderRadius: '6px',
              background: viewMode === 'map' ? 'rgba(59,130,246,0.1)' : 'transparent',
      ...
      <div className="glass-card reveal" style={{ padding: '14px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div style={{ display: 'flex', gap: '10px', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
          <div style={{ position: 'relative', flexGrow: 1, minWidth: '200px' }}>
            <Search size={16} style={{ position: 'absolute', left: '12px', top: '14px', color: 'var(--text-muted)' }} />
            <input 
              type="text" 
              placeholder="Search challenges by keywords, ID (e.g. monsoon-road-accessibility)..."
              value={search}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <select 
              ...
              style={{ padding: '10px 32px 10px 12px', fontSize: '0.85rem', width: '160px', background: 'rgba(0,0,0,0.15)' }}
            >
      ...
      {viewMode === 'map' ? (
        ...
        <div>
          ...
          <div style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginBottom: '16px', display: 'flex', justifyContent: 'space-between' }}>
            <span>Found <strong>{filteredChallenges.length}</strong> active challenges waiting for review.</span>
          ...
          <div className="reveal-stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: '20px' }}>
            {filteredChallenges.map(item => (              <div key={item.id} className="glass-card" style={{ 
                  display: 'flex', 
                  flexDirection: 'column', 
                  justifyContent: 'space-between',
                  minHeight: '300px',
                  borderTop: `3px solid ${getSeverityColor(item.severity)}`,
                  borderTopLeftRadius: '2px',
                  borderTopRightRadius: '2px',
                }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', fontFamily: 'var(--font-display)', fontWeight: 600 }}>ID: #{item.id}</span>
                  ...
                  <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '14px', lineHeight: '1.4' }}>
                    {item.description}
                  </p>

                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }}>
                    {item.skills_required.map((skill, sIdx) => (
                      <span key={sIdx} style={{ fontSize: '0.7rem', color: 'var(--text-secondary)', background: 'rgba(255,255,255,0.04)', padding: '2px 8px', borderRadius: '4px', border: '1px solid var(--border-subtle)' }}>
                        {skill}
                      </span>
                    ))}
                  </div>
                ...
                <div style={{ borderTop: '1px solid var(--border-subtle)', paddingTop: '14px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                    ...
                    <div style={{ fontWeight: 600, color: getSeverityColor(item.severity) }}>
          ...
          {filteredChallenges.length === 0 && (
            <div className="glass-card reveal" style={{ padding: '60px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
              ...
              <p style={{ color: 'var(--text-secondary)', maxWidth: '400px', margin: '0 auto', fontSize: '0.85rem' }}>
                We couldn't find any challenges in Jharkhand matching your search query. Try choosing 'All Districts' or reporting a new one.
              </p>
